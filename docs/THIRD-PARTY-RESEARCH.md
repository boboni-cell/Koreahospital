...
# 本机修复版 60s Worker（小红书/抖音榜单）
SIXTY_SECONDS_API_BASE=http://127.0.0.1:4399
```
...
研究热点页点击“刷新”或输入关键词点击“筛选趋势”时，Koreahospital 会先通过 MCP `trigger_crawl(save_to_local=true)` 拉取最新平台数据，再调用 `search_news` 返回结果；项目会在配置 `TRENDRADAR_DIR` 时自动拉起本机 MCP。小红书和抖音榜单只接受 60s 的榜单响应；服务离线或没有数据时显示清晰错误/空结果，不把 socai 搜索结果冒充热榜。TrendRadar 的配置、新闻库、通知和 AI Key 都留在其独立服务内。
