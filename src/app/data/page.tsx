...
export default function DataPage() {
  ...
  return (
    <PageFrame>
      <h2 className="mb-4 text-xl font-semibold tracking-tight text-[#01011b]">数据看板</h2>
      {loading ? (
        ...
        <div className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Stat title="增长率" value={`${m?.growth ?? 0}%`} tone="text-[#6f63b7]" />
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <Card>
              <CardHeader>
                <CardTitle className="text-sm font-medium text-[#43394c]">粉丝增长趋势（近 14 天）</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-sm font-medium text-[#43394c]">每日点赞 vs 播放</CardTitle>
              </CardHeader>
              <CardContent className="space-y-2">
                <div>
                  <div className="mb-1 flex items-center gap-2 text-xs text-[#717a94]">
...
function Stat({ title, value, tone }: { title: string; value: string; tone: string }) {
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-sm font-normal text-[#717a94]">{title}</CardTitle>
      </CardHeader>
      <CardContent>
