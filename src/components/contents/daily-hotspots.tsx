...
export function DailyHotspots() {
  ...
  function askCodexResearch(item: HotspotItem & { sourceName: string }) {
    window.dispatchEvent(new CustomEvent("toni:compose", { detail: { text: `[使用 Codex 研究工具] 请深度查看并核验${item.sourceName}热点「${item.title}」的公开网页信息，整理证据、趋势变化和 5 个合规选题。来源：${item.link || "60s API"}` } }));
  }
