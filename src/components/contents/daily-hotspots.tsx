...
export function DailyHotspots() {
  ...
  return <div className="space-y-5">
    ...
    {loading ? <div className="flex items-center justify-center py-20 text-sm text-[#89828d]"><Loader2 className="mr-2 h-4 w-4 animate-spin" /> 正在读取热点…</div> : items.length ? <div className="overflow-hidden rounded-2xl border border-[#e4ddd6] bg-white shadow-[0_16px_50px_rgba(53,45,38,.07)]">{items.map((item, index) => { const style = sourceStyles[item.sourceId]; const topThree = item.rank <= 3; return <div key={`${item.sourceId}-${item.rank}-${item.title}`} className={`group grid grid-cols-[44px_1fr] gap-3 px-4 py-4 transition-colors hover:bg-[#fbf9f7] sm:grid-cols-[44px_40px_1fr_auto] sm:items-center sm:px-5 ${index ? "border-t border-[#eee8e2]" : ""}`}>
      ...
      <div className="col-start-2 flex gap-2 sm:col-start-auto sm:opacity-70 sm:transition-opacity sm:group-hover:opacity-100">{item.link && <a href={item.link} target="_blank" rel="noreferrer"><Button size="sm" variant="ghost"><ExternalLink className="h-3.5 w-3.5" /> 来源</Button></a>}<Button size="sm" onClick={() => askToni(item)}><Sparkles className="h-3.5 w-3.5" /> Toni 想选题</Button></div>
    </div>; })}</div> : <div className="rounded-xl border border-dashed p-12 text-center text-sm text-[#89828d]">当前没有可用热点，请稍后刷新或配置自建 60s API。</div>}
